    ...
    void SimpleIndex::insertItems(span<block> items, block hashingSeed)
    {
        ...
        if (mNumHashFunctions == 3 )
        {
            for (u64 i = 0; i < mainSteps; ++i, itemIdx += 8)
            {
                ...
                auto bIdx00 = old::CuckooIndex<>::getHash(hashs[0], 0, mNumBins);
                auto bIdx10 = old::CuckooIndex<>::getHash(hashs[1], 0, mNumBins);
                auto bIdx20 = old::CuckooIndex<>::getHash(hashs[2], 0, mNumBins);
                auto bIdx30 = old::CuckooIndex<>::getHash(hashs[3], 0, mNumBins);
                auto bIdx40 = old::CuckooIndex<>::getHash(hashs[4], 0, mNumBins);
                auto bIdx50 = old::CuckooIndex<>::getHash(hashs[5], 0, mNumBins);
                auto bIdx60 = old::CuckooIndex<>::getHash(hashs[6], 0, mNumBins);
                auto bIdx70 = old::CuckooIndex<>::getHash(hashs[7], 0, mNumBins);
                ...
                auto bIdx01 = old::CuckooIndex<>::getHash(hashs[0], 1, mNumBins);
                auto bIdx11 = old::CuckooIndex<>::getHash(hashs[1], 1, mNumBins);
                auto bIdx21 = old::CuckooIndex<>::getHash(hashs[2], 1, mNumBins);
                auto bIdx31 = old::CuckooIndex<>::getHash(hashs[3], 1, mNumBins);
                auto bIdx41 = old::CuckooIndex<>::getHash(hashs[4], 1, mNumBins);
                auto bIdx51 = old::CuckooIndex<>::getHash(hashs[5], 1, mNumBins);
                auto bIdx61 = old::CuckooIndex<>::getHash(hashs[6], 1, mNumBins);
                auto bIdx71 = old::CuckooIndex<>::getHash(hashs[7], 1, mNumBins);
                ...
                auto bIdx02 = old::CuckooIndex<>::getHash(hashs[0], 2, mNumBins);
                auto bIdx12 = old::CuckooIndex<>::getHash(hashs[1], 2, mNumBins);
                auto bIdx22 = old::CuckooIndex<>::getHash(hashs[2], 2, mNumBins);
                auto bIdx32 = old::CuckooIndex<>::getHash(hashs[3], 2, mNumBins);
                auto bIdx42 = old::CuckooIndex<>::getHash(hashs[4], 2, mNumBins);
                auto bIdx52 = old::CuckooIndex<>::getHash(hashs[5], 2, mNumBins);
                auto bIdx62 = old::CuckooIndex<>::getHash(hashs[6], 2, mNumBins);
                auto bIdx72 = old::CuckooIndex<>::getHash(hashs[7], 2, mNumBins);
            ...
            for (u64 i = 0; i < remSteps; i += hashs.size())
            {
                ...
                {
                    auto bIdx = old::CuckooIndex<>::getHash(hashs[i], (u8)h, mNumBins);
        ...
        else
        {
            ...
            for (u64 i = 0; i < u64(items.size()); i += u64(hashs.size()))
            {
                ...
                for (u64 j = 0, itemIdx = i; j < min; ++j, ++itemIdx)
                {
                    ...
                    for (u64 h = 0; h < mNumHashFunctions; ++h)
                    {
                        auto bIdx = old::CuckooIndex<>::getHash(hashs[j], (u8)h, mNumBins);
                        bool collision = false;
