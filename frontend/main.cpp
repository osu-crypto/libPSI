...
void ttt22()
{
	std::vector<std::array<u64, 2>> params{ { 16,8 },{ 18,8 } };
	for (u64 numKeys : {128})
	{
		std::cout << "nk " << numKeys << std::endl;
		for (auto param : params)
		{
			...
			BgiPirServer::MultiKey mk0;
			mk0.init(k0, g0);

			Timer tt;
			auto ss = tt.setTimePoint("s");
			for (u64 i = 0; i < domain; ++i)
			{
				auto bits0 = mk0.yeild();
			}
			auto ee = tt.setTimePoint("e");

			auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(ee - ss).count() / 1000.0;

			std::cout << ms << "s " << "domain " << domain << "   rate: " << u64((domain * numKeys) / (double(ms))) << std::endl;

			ss = tt.setTimePoint("s");
			BgiPirServer::fullDomainNaive(data, k0[0], g0[0]);
			ee = tt.setTimePoint("e");
			ms = std::chrono::duration_cast<std::chrono::milliseconds>(ee - ss).count() / 1000.0;
			std::cout << ms << "s " << "domain " << domain << "   rate: " << u64((domain) / (double(ms))) << std::endl;
